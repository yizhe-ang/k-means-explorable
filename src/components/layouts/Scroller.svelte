...
<script>
  ...
  function setFixed() {
    if (bgContainer) {
      ...
      let style = `position: ${
        fixed ? "fixed" : "absolute"
      }; top: 0; transform: translate(0, ${offset_top}px); z-index: ${
        inverted ? 3 : 1
      };`;
      // Don't set width
...
<!--
@component
From: https://github.com/ONSvisual/scrolly_AML_robo_animation_svelteKit
Patched to transpile to IE 11 and allow for split-screen view option.
Based on: https://github.com/sveltejs/svelte-scroller
 -->
